...
class HISCORE {
  ...
  String scorelistFilename = System.getProperty("user.home") + 
      "/VideoGameCabinetData/HighScore_ROCKETCOMMAND.txt";

  String [] scoreList = new String[10];


  HISCORE() {
    scoreFont = myScene.font;
  ...
  void shiftScores(int tempRank) {
    ...
    if (tempRank < 10) {
      scoreList[tempRank] = "          ,"+myCursor.score;
    }
  }
  ...
  void topFive() {
    ...
    for (int i = 0; i < 5; i++) {
      textAlign(LEFT);
      String [] scoreLine = split(scoreList[i], ',');
      text(scoreLine[0], width/2-xF*12, height/2 + 11*yF+(35*i));
      text(scoreLine[1], width/2+xF*10, height/2 + + 11*yF+(35*i));
    }
    textFont(scoreFont, xF*3);
    textAlign(CENTER);
    fill(fontColor);
    text("TOP FIVE", width/2, height/2+8*yF);
  }
